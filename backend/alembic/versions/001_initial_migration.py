...
def upgrade() -> None:
    ...
    op.create_table('parsed_appointments',
        ...
        sa.CheckConstraint("appointment_type IN ('General Checkup', 'Dental', 'Vision', 'Specialist', 'Vaccination', 'Follow-up', 'Emergency', 'Lab Work', 'Physical Therapy', 'Mental Health', 'Veterinary', 'Other')", name='parsed_appointment_type_check'),
        sa.CheckConstraint("processing_status IN ('completed', 'failed', 'processing')", name='processing_status_check')
    )
    ...
    # Apply triggers
    op.execute("CREATE TRIGGER update_parsed_appointments_updated_at BEFORE UPDATE ON parsed_appointments FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();")

    # Add comments
    op.execute("COMMENT ON TABLE parsed_appointments IS 'Stores parsed appointment data extracted from uploaded PDF medical documents';")


def downgrade() -> None:
    # Drop triggers
    op.execute("DROP TRIGGER IF EXISTS update_parsed_appointments_updated_at ON parsed_appointments;")
